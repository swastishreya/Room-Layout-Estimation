...
def simple_distance_loss(pred, target, coordinate_list, use_gpu, info, umbrella_size=2):
    loss_mask = np.zeros(target.shape, dtype='uint8')

    if info['epoch'] == 0:
        for target_y_cor, target_x_cor in coordinate_list:
            for x_cor in range(min(0,target_x_cor-umbrella_size),min(512,target_x_cor+umbrella_size)):
                for y_cor in range(min(0,target_y_cor-umbrella_size),min(512,target_y_cor+umbrella_size)):
